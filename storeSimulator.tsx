...
export default function storeSimulator() {
  ...
  let success = useSelector((state: RootState) => state.success);
  let failed = useSelector((state: RootState) => state.failed);
  ...
  async function sendPushNotification(expoPushToken: string, status: string, success: number, failed: number) {
    ...
    const message = {
      ...
      sound: 'default',
      title: `${status}`,
      body: `${success} successful, ${failed} unsuccessful.`,
  ...
  const addData = async () => {
    ...
    try {
      ...
      await sendPushNotification(expoPushToken, 'success', success, failed);
      if (Platform.OS === "android") {
      ...
      await sendPushNotification(expoPushToken, 'failed', success, failed);
    }
  }
